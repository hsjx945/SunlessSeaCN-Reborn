...
当前兼容包版本：**6.0.2**。项目不包含《Sunless Sea》游戏本体或游戏原始资源，使用前请先通过 Steam 等合法渠道拥有并安装游戏。

## 快速下载

从 [GitHub Releases](https://github.com/hsjx945/SunlessSeaCN-Reborn/releases/latest) 下载对应平台的压缩包：

- `SunlessSeaCN-Windows-v6.0.2.zip`：Windows 一键安装、备份、重复安装和卸载恢复。
- `SunlessSeaCN-macOS-v6.0.2.zip`：macOS universal 一键安装并启动；修复 ZIP 解压后 `.command` 执行权限丢失问题，macOS 实机运行仍需首次验证。
...
下面是本项目实际测试所对应的游戏版本，不要把补丁版本 `6.0.2` 误认为游戏版本号：
...
Windows 实机日志已确认 BepInEx 和中文插件正常加载，并应用 Sunless Sea 的 Harmony 补丁。若 Steam 将游戏更新到新的 BuildID 或 Unity 主版本，**不能保证 6.0.2 仍然可用**；遇到插件不加载、启动崩溃或文本失效时，应等待本仓库发布兼容更新。
...
1. 解压 `SunlessSeaCN-Windows-v6.0.2.zip`。
...
1. 解压 `SunlessSeaCN-macOS-v6.0.2.zip`。安装包可以放在下载文件夹、桌面或其他本地目录，不需要放进游戏目录。
...
可以。macOS v6.0.2 解压后双击 `Install-And-Start-SunlessSeaCN.command`，会自动安装汉化并直接通过 BepInEx 启动游戏，不需要设置 Steam 启动项。Steam 客户端未运行时，脚本会尝试自动打开它。macOS 对未签名的下载文件仍可能显示一次系统安全确认，这是系统的正常保护提示，无法由补丁安全地永久绕过。
